...
cdef extern from "clipper.hpp" namespace "ClipperLib":
    ...
    bint Orientation "Orientation"(const Path & poly)
    double Area "Area"(const Path & poly)
    int PointInPolygon "PointInPolygon"(const IntPoint & pt, const Path & path)
    ...
    void SimplifyPolygon "SimplifyPolygon"(const Path & in_poly, Paths & out_polys, PolyFillType fillType)
    void SimplifyPolygons "SimplifyPolygons"(const Paths & in_polys, Paths & out_polys, PolyFillType fillType)
    void CleanPolygon "CleanPolygon"(const Path& in_poly, Path& out_poly, double distance)
    void CleanPolygons "CleanPolygons"(Paths& polys, double distance)

    void MinkowskiSum "MinkowskiSum"(const Path& pattern, const Path& path, Paths& solution, bint pathIsClosed)
    void MinkowskiSum "MinkowskiSum"(const Path& pattern, const Paths& paths, Paths& solution, bint pathIsClosed)
    void MinkowskiDiff "MinkowskiDiff"(const Path& poly1, const Path& poly2, Paths& solution)

    void PolyTreeToPaths "PolyTreeToPaths"(const PolyTree& polytree, Paths& paths)
    void ClosedPathsFromPolyTree "ClosedPathsFromPolyTree"(const PolyTree& polytree, Paths& paths)
    void OpenPathsFromPolyTree "OpenPathsFromPolyTree"(PolyTree& polytree, Paths& paths)

    void ReversePath "ReversePath"(Path& p)
    void ReversePaths "ReversePaths"(Paths& p)
