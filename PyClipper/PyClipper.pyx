...
cdef extern from "clipper.hpp" namespace "ClipperLib":
    ...
    bint c_Orientation "Orientation"(const Path & poly)
    ...
    void c_MinkowskiSum "MinkowskiSum"(const Path& pattern, const Path& path, Paths& solution, bint pathIsClosed)
    void c_MinkowskiSum "MinkowskiSum"(const Path& pattern, const Paths& paths, Paths& solution, bint pathIsClosed)
